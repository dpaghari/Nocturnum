...
public class CanSpawn : MonoBehaviour {
	 ...
	 * LevelInfo contains how many units are spawned and how many times it gets repeated
	 * and when it starts
	...
	*/
	...
	void Start(){
		...
		addSpawner (25.0F, 1.0F, 25.0F); addSpawner (-25.0F, 1.0F, -25.0F); addSpawner (-55.0F, 1.0F, 15.0F);
		SpawnInfo initial = spawnLocations.Find(x => x.nodeName.EndsWith("1")); initial.turnOn ();
		addLevel (1, 3, 2, 25.0F); addLevel (2, 4, 2, 20.0F); addLevel (3, 5, 2, 25.0F); 
		addLevel (4, 3, 2, 25.0F); addLevel (5, 4, 2, 30.0F); addLevel (6, 3, 2, 35.0F);
	...
	public void addSpawner(float _x, float _y, float _z){
		numLocations++;
	...
	public void addLevel(int nLevel, int nSpawn, int tToRun, float sTime){
